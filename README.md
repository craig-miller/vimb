...
13. **`handler-add` short-circuit on new-window paths.** Upstream's `decide_navigation_action` already consults the `:handler-add` registry — a URI whose scheme has a registered handler fires the script and the WebKit decision is ignored. The two new-window entry points (`on_webview_create` for target=\_blank clicks and SVG anchors in embedded viewers, `decide_new_window_action` for CTRL-click / middle-click / `FLAG_NEW_WIN`) did NOT consult the handler and always fell through to `spawn_new_instance` — leaving a blank vimb window every time a scheme-handler URI was clicked with any "open in new window" affordance. Concretely: `handler-add sioyek=/path/to/proxy %s` correctly routed same-window `sioyek://…` clicks to the proxy but *also* spawned a blank vimb per click when the containing page rendered the link inside an SVG (as typst-preview does for `#link(...)`). Added the same `handler_handle_uri` short-circuit at both new-window sites so handler routing is uniform regardless of how the click would have opened the URI.
